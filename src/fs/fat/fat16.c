...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (diskstream_seek(stream, directory_start_pos) != PEACHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstream_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != PEACHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    directory->ending_sector_pos = root_dir_sector_pos + root_dir_size / disk->sector_size; // Buggy
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstream_seek(stream, fat_table_position * (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
struct fat_item *fat16_new_fat_item_for_directory_item(struct disk *disk, struct fat_directory_item *item)
{
    ...
    f_item->type = FAT_ITEM_TYPE_FILE; // Buggy
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    for (int i = 0; i < directory->total; ++i)
    {
        ...
        if (isstrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0)
        {
